...
function [] = bobbySegmentCardiomyo(path,NucMaskChan,Row,Col,SaveFig)   
...
% 
% ["/home/rps21/Cardiomyocite/segmentation/bobbySegmentCardioMyo/run_bobbySegmentCardiomyo_orchestra_addbackdapi.sh", "/home/rps21/MCR/v901/", 'scratch2/rps21/ResegmentedPlates/%s' % platename, nuclearstack, rows, cols])


% path 'scratch2/rps21/ResegmentedPlates/%s'
%      nuclearstack = '[1 5]'
%     rows = 'BCDEFG' CHANGE IN PYTHON - '['B' 'C' 'D']'
%     cols = '[2:11]'
% platename = plate (Processed....)

%Old: bobbySegmentCardiomyo(inputpath,nucleistack,rows,cols)


javaaddpath('/home/rps21/Cardiomyocite/segmentation/bobbySegmentCardioMyo/bfmatlab/bioformats_package.jar')
...
% ip.addParamValue('Row',['A' 'H']);  
% % ip.addParamValue('Col',[1 12],@(x)(numel(x) == 2 & all(x > 0 ))); 
% ip.addParamValue('Col',[1 12]); 
...
% testPath = p.Path;
testPath = path;
NucMaskChan = str2num(NucMaskChan);
% row = p.Row(1):p.Row(2);
row = Row;
% row = rows(1):rows(2);
