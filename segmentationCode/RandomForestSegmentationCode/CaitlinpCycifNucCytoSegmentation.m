function CaitlinpCycifNucCytoSegmentation(varargin)
...
% modelPath = [drive ':\sorger\data\IN Cell Analyzer 6000\Connor\MCF10 Common\Real experiment\All Cyles\nucleiTrainingSet10x'];
                     ...
                     for iFile = 1:numel(files)
                        ...
                        %% apply random forest model to generate classProbs
                            ...
                            %% markers based on log filter on classProbs 3
                             logNuclei=  gather(imgaussfilt3(gpuArray(filterLoG(nucleiClass,4)),2.5)); %3.5, 2
...
%                           figure,imshowpair(logfgm,sqrt(normalize(imresize(nucleiClass,1))))
                          ...
                          %% contours for watershed
                            nucleiBlur = gather(medfilt2(gpuArray(nucleiImage),[11 11]));
                            hy = fspecial('sobel');
                            hx = hy';
                        ...
                        if sum(sum(multiNuclei))>0
                            ...
                            Imax = gather(imregionalmin(gpuArray(imgauss)));
